...
def solvef(
    rs, lmbda, doplot=False
):  # Solves the second order equation for density normalization function f (eq. 16)
    def func(u, x):
...
def CalculateSpecificInternalEnergy(rho, k, gamma):
    ...
    for i in range(len(rho[:, 0])):
        for j in range(len(rho[0, :])):
            eps[i][j] = k / (gamma - 1.0) * rho[i][j] ** (gamma - 1.0)
    return eps
...
def GenerateProfile(savedata=False):
    ...
    t = 1.0 * np.ones(1)
    lmbda = 0.002
    rs = np.linspace(0.001, 10.0, 10000)
    cv = 1.0
    gamma = 4.0 / 3.0
    ...
    for n in range(len(r)):
        rhoe[n] = rho[0, n] + rho[0, n] * eps[0, n] / c ** 2.0
    mass = 0
    for i in range(len(r) - 1):
        mass = mass + 2.0 * np.pi * (
            rho[0, i + 1] * (a[0] * r[i + 1]) ** 2.0 + rho[0, i] * (a[0] * r[i]) ** 2.0
        ) * a[0] * (r[i + 1] - r[i])
